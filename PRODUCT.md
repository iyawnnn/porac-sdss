...
WCAG AA contrast (4.5:1 body text, 3:1 large text/UI) across all token pairs. All motion respects `prefers-reduced-motion`. **There is no dark-mode toggle, and none is currently planned** — both the admin and citizen shells currently render a single light theme. Citizen touch targets are a hard 44×44px floor with 16px minimum input text (below 16px, iOS Safari zooms on focus — disorienting for a one-handed outdoor report). Urgency color-coding carries a non-color-dependent fallback (spelled-out band names, pin size/ring) for colorblind users.
