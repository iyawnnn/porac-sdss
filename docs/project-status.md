...
- **Wider fixture sharing — done for the read-only slice.** `admin-tickets.spec.ts`'s three purely-read tests (queue→detail nav, Ticket Detail read-only sections, mobile card list) now share one `sharedReadOnlyTicketId` fixture, cutting that file's report-creating call sites from 8 to 6 and the full-suite total from ~17 to ~15 at the time this shipped (see [`testing.md`](testing.md) §5–§6). Its remaining call sites all mutate status, office, or resolution one-way and correctly stay isolated — no further reduction is possible there without database isolation. **Numbers have since moved again**: the ticket-reassignment security tests (§3 above) added one more disposable-ticket call site afterward, so the file currently sits at 7 and the full-suite total at ~17 — `testing.md` §6's table is the current authoritative count; treat the numbers in this bullet as a historical snapshot of the 016 fix itself, not today's total.
- **Playwright in CI — pending**, and correctly gated behind database isolation. CI today runs two parallel jobs (Frontend: typecheck, lint, build; API: build-recovery check, build, unit tests) — no browser tests. See [`testing.md`](testing.md) §1.
