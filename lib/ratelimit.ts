...
export async function checkRateLimit(
  ...
  if (accountHourlyCount >= ACCOUNT_HOURLY_LIMIT) {
    return { allowed: false, reason: `Too many reports from your account this hour. Try again later.` };
  }
  ...
  if (ipHourlyCount >= IP_HOURLY_BACKSTOP) {
    return { allowed: false, reason: `Too many reports from this network. Try again later.` };
  }
