...
// J/K to move, X to select, Enter to open the review drawer — the same
// mail-client convention the Ticket Queue uses, so the two admin tables never
// answer the same keystroke differently.
//
...
// search box would move the focus ring three times and select a row — and the
// search box is the most-used control on the page.
export function useFlaggedKeyboard({
  ids,
  onToggleSelect,
  onOpen,
  enabled = true,
  ...
  useEffect(() => {
    ...
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (key === "j" || key === "k") {
        ...
        setFocusedId(nextId);
        document
          .querySelector(`[data-report-row="${nextId}"]`)
