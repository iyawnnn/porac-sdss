...
- **Urgency** (`urgency_score`, `urgency_band`) is the system-computed environmental hazard score from the Urgency triage formula above (elevation + rain + cluster density). `priority_score` and `urgency_level` are the *same* `urgency_score`, just rescaled 0–100 (`priorityScore = round(urgencyScore * 100)`) and banded LOW/MEDIUM/HIGH at 50/80 — they are an urgency representation, not a separate concept, and are labeled "Urgency" in the UI (Dashboard, Ticket Queue, Ticket Detail) despite the `priority_score` column name. `urgency_band` is a Title-Case restatement of `urgency_level` (`urgencyLevelFromScore`, `api/src/domain/urgency.ts`), not an independently-thresholded value, so the two can never disagree — this was previously a real discrepancy (band cut at 0.4/0.7 with a third label of "Critical"; level cut at 50/80 with "HIGH") and has been resolved by deriving `urgency_band` from `urgency_level` instead of computing it separately. "Critical" is no longer a Hazard Urgency label anywhere in the system; it remains a valid citizen-reported severity value only.
