...
export async function getTicketsForAdmin(filters: AdminTicketFilters = {}) {
  return sql<AdminTicketRow[]>`
    SELECT
      t.id, t.category, t.barangay_id, b.name AS barangay_name,
      t.member_count, t.urgency_score, t.urgency_band, t.assigned_office,
    ...
    WHERE (${filters.office ?? null}::text IS NULL OR t.assigned_office = ${filters.office ?? null}::office)
      AND (${filters.status ?? null}::text IS NULL OR t.status = ${filters.status ?? null}::ticket_status)
