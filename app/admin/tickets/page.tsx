...
export default async function AdminTicketsPage({
  ...
  searchParams: Promise<{ office?: string; status?: string; barangayId?: string; urgency?: string }>;
}) {
  ...
  // toggle; ?office=MEO/MDRRMO overrides the default either direction.
  const office =
    params.office === "all"
      ? undefined
      : params.office === "MEO" || params.office === "MDRRMO"
        ? params.office
        : session?.office;
  const status = params.status === "all" || params.status === "active" || (params.status && STATUSES.includes(params.status)) ? params.status : "active";
  ...
  const [tickets, barangays] = await Promise.all([
    getTicketsForAdmin({ office, status: status as NonNullable<Parameters<typeof getTicketsForAdmin>[0]>["status"], barangayId, urgency }),
